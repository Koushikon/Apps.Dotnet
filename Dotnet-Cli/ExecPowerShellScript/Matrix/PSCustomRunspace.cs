...
public class PSCustomRunspace
{
    ...
    public PSCustomRunspace()
    {
        ...
        var entry = new SessionStateVariableEntry("AllowedCommands", new[] { "Get-Date" }, "List of allowed commands");
        iss.Variables.Add(entry);

        var ms = new ModuleSpecification("Microsoft.PowerShell.Utility");
        iss.ImportPSModule(new[] { ms });

        var getDateCmdLet = new SessionStateCmdletEntry("Get-Date", typeof(GetDateCommand), "");
